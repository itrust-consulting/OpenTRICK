...
function deleteIDS() {
	...
	try {
		$confirmModal.find(".modal-body").html(
			selectedIDS.length > 1 ? MessageResolver("confirm.delete.multi.ids", "Are you sure you want to delete selected IDSs") : MessageResolver(
				"confirm.delete.single.ids", "Are you sure, you want to delete selected IDS"));
...
function renewIDSToken() {
	...
	try {
		$confirmModal.find(".modal-body").html(
			selectedIDS.length > 1 ? MessageResolver("confirm.renew.multi.ids.token", "Are you sure you want to renew the tokens of selected IDS") : MessageResolver(
				"confirm.delete.single.ids.token", "Are you sure you want to renew token of selected IDS"));
		$confirmModal.find(".modal-footer>button[name='yes']").one("click", function (e) {
			$progress.show();
			$.ajax({
				...
				success: function (response, textStatus, jqXHR) {
					if (response["error"] != undefined)
						...
						showDialog("#info-dialog", MessageResolver("error.unknown.renew.ids.token", "An unknown error occurred while renewing token of IDS"));
