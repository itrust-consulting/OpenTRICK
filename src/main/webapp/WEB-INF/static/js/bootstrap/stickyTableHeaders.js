...
(function($, window, undefined) {
	...
	function Plugin(el, options) {
				...
				.debounce(
						function(e) {
							if (base.$el) {
								base.$el
										.each(function() {
											var $this = base.$el, topPosition = (base.topPosition == null || !base.cacheHeaderHeight || e.type == 'resize' ? base.topPosition = (isNaN(base.options.fixedOffset) ? base.options.fixedOffset
													.outerHeight()
													+ base.options.fixedOffset.position().top
											...
											scrolledPastTop = base.isWindowScrolling ? scrollTop * base.options.scrollStartFixMulti > offset.top : newTopOffset > offset.top, notScrolledPastBottom = (base.isWindowScrolling ? scrollTop
													: 0) < (offset.top + $this.height() - headerHeight - (base.isWindowScrolling ? 0 : newTopOffset));

											if (scrolledPastTop && notScrolledPastBottom) {

												var newLeft = offset.left - scrollLeft + base.options.leftOffset, state = base.isSticky, doSet = !base.options.setStylesOnce
														|| !base.isSticky || newLeft != base.leftOffset;
		...
		base.updateWidth = function() {
			...
			// If we're caching the height, we need to update the cached value
