			...
			<table class="table table-hover">
				...
				<tbody>
					...
					<c:if test="${showExcludeDynamic and not empty excludeAcronyms}">
						...
						<c:forEach items="${excludeAcronyms.entrySet()}" var="entry" varStatus="status">
							<spring:message text="${entry.key}" var="acronym"/>
							<tr data-trick-class="ExcludeDynamicParameter" data-trick-id="${acronym}">
								<td class="textaligncenter">${acronym}</td>
								<td class="textaligncenter"><fmt:formatNumber value="${entry.value}" maxFractionDigits="4" minFractionDigits="4" /></td>
								<c:if test="${isEditable}">
