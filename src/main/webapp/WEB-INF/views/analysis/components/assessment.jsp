	...
	<div class="panel panel-default">
		<div class="panel-heading">
			<button type="button" class="btn btn-default"
				onclick="return generateMissingAssessment();">
		...
		<div class="panel-body carousel">
			<table class="table table-responsive" trick-table="1">
				<thead>
					<tr>
						<th colspan="3" trick-table-part="0"><spring:message
								code="label.assessment.scenario" text="Scenario"
								htmlEscape="true" /></th>
						<th trick-table-part="1"><spring:message
								code="label.assessment.impactRep" text="Rep." htmlEscape="true" /></th>
						...
						<th trick-table-part="2"><spring:message
								code="label.assessment.ALEP" text="ALEP" htmlEscape="true" /></th>
						<th><spring:message code="label.assessment.ALE" text="ALE"
								htmlEscape="true" /></th>
						<th><spring:message code="label.assessment.ALEO" text="ALEO"
								htmlEscape="true" /></th>
						<th colspan="5" trick-table-part="3"><spring:message
				...
				<tbody>
					...
					<c:forEach items="${sortedAssessments}" var="assessment">
						<c:if test="${empty(prevAsset) || assessment.asset !=prevAsset }">
							<tr class="warning">
								<td colspan="12" trick-table-part="0"
									title="${assessment.asset.name}">${assessment.asset.name}</td>
							</tr>
						</c:if>
						<tr>
							...
							<td ondblclick="return editField(this, 'assessment','${assessment.id}', 'uncertainty', 'double');" real-value="${assessment.uncertainty}"><fmt:formatNumber value="${assessment.uncertainty}"
			...
			<a href="#" class="carousel-control no-background left"
				control-trick-table="left"> <span class="icon-prev"></span>
			</a> <a href="#" class="carousel-control no-background right"
				control-trick-table="right"><span class="icon-next"></span></a>
		</div>
	</div>
</div>