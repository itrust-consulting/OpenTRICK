						...
						<tr trick-class="Assessment" trick-id="${assessment.id}" trick-callback="chartALE()">
							...
							<c:choose>
								<c:when test="${parameters.containsKey(assessment.impactFin)}">
									<td trick-field="impactFin" trick-field-type="string" class="success" title='<fmt:formatNumber value="${parameters.get(assessment.impactFin)}" />'
										ondblclick="return editField(this);"><spring:message text="${assessment.impactFin}" htmlEscape="true" /></td>
								</c:when>
								<c:otherwise>
									<td trick-field="impactFin" trick-field-type="string" class="success" ondblclick="return editField(this);" title="${assessment.impactFin}"><spring:message text="${assessment.impactFin}"
											htmlEscape="true" /></td>
								</c:otherwise>
							</c:choose>
							<c:choose>
