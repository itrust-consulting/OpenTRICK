...
<div class="section" id="section_measure">
	...
	<c:forEach items="${measureSplited.keySet()}" var="norm">
		<div id="section_measure_${norm}" class="sectionpanel">
							...
							<c:forEach items="${measureSplited.get(norm)}" var="measure">
								<c:choose>
									<c:when test="${measure.measureDescription.level<3 }">
										<tr
											${measure.measureDescription.level<2? "class='danger'" : "class='warning'" }>
											<td><spring:message
									...
									<c:otherwise>
										<tr trick-class="Measure" trick-id="${measure.id}"
											...
											<td ${measure.cost == 0? "class='danger'" : "" }
												title="${measure.cost}"><fmt:formatNumber
