				...
				<table class="table table-hover">
					<thead>
						<tr>
							...
							<td colspan="2"><spring:message code="label.measure.domain" text="Domain" /></td>
							...
							<td><spring:message code="label.measure.inv" text="INV" /> (k&euro;)</td>
							<td><spring:message code="label.measure.lt" text="LT (y)" /></td>
							<td><spring:message code="label.measure.mt" text="MT (%)" /></td>
							<td><spring:message code="label.measure.cs" text="CS" /> (k&euro;)</td>
							<td><spring:message code="label.measure.phase" text="Phase" /></td>
							<td colspan="2"><spring:message code="label.measure.comment" text="Comment" /></td>
							<td colspan="2"><spring:message code="label.measure.todo" text="To do" /></td>
						</tr>
					</thead>
					<tbody>
							...
							<c:choose>
								<c:when test="${measure.measureDescription.level<3 }">
									<tr ${measure.measureDescription.level<2? "class='danger'" : "class='warning'" }>
										...
										<td colspan="14"><spring:message text="${!empty measureDescriptionText? measureDescriptionText.domain : ''}" /></td>
									</tr>
								</c:when>
								<c:otherwise>
									<tr trick-class="Measure" trick-id="${measure.id}" trick-callback="reloadMeasureRow('${measure.id}','${norm}')">
										...
										<td class="success" trick-field="status" trick-choose="M,AP,NA" trick-field-type="string" ondblclick="return editField(this);"><spring:message
												text="${measure.status}" htmlEscape="true" /></td>
										<c:choose>
											<c:when test="${norm.equalsIgnoreCase('Custom')==true}">
												<td class="success" trick-field="implementationRate" trick-field-type="double" ondblclick="return editField(this);"><spring:message
														text="${measure.getImplementationRateValue()}" /></td>
											</c:when>
											<c:when test="${norm.equalsIgnoreCase('Maturity')==false}">
												<td class="success" trick-field="implementationRate" trick-field-type="double" trick-callback="reloadMeausreAndCompliance('${norm}','${measure.id}')"
													ondblclick="return editField(this);"><spring:message text="${measure.getImplementationRateValue()}" /></td>
											</c:when>
											<c:otherwise>
												<td class="success" trick-field="implementationRate" trick-field-type="double" ondblclick="return editField(this);" trick-class="MaturityMeasure"
													trick-id="${measure.id}"><spring:message text="${measure.getImplementationRateValue()}" /></td>
											</c:otherwise>
										</c:choose>
										<td class="success" trick-field="internalWL" trick-field-type="double" ondblclick="return editField(this);"><spring:message text="${measure.internalWL}" /></td>
										<td class="success" trick-field="externalWL" trick-field-type="double" ondblclick="return editField(this);"><spring:message text="${measure.externalWL}" /></td>
										<td class="success" trick-field="investment" trick-field-type="double" ondblclick="return editField(this);"><spring:message text="${measure.investment}" /></td>
										<td class="success" trick-field="lifetime" trick-field-type="double" ondblclick="return editField(this);"><spring:message text="${measure.lifetime}" /></td>
										<td class="success" trick-field="maintenance" trick-field-type="double" ondblclick="return editField(this);"><spring:message text="${measure.maintenance}" /></td>
								...
								${measure.phase.number}
										...
										<td colspan="2" class="success" trick-field="comment" trick-field-type="string" ondblclick="return editField(this);"><spring:message text="${measure.comment}" /></td>
										<td colspan="2" class="success" trick-field="toDo" trick-field-type="string" ondblclick="return editField(this);"><spring:message text="${measure.toDo}" /></td>
									</tr>
								</c:otherwise>
							</c:choose>
						</c:forEach>
