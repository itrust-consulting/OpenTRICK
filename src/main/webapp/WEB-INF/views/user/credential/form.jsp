...
<div class="modal fade" id="credential-modal-form" tabindex="-1"
	role="dialog" data-aria-labelledby="credential-modal-form"
	data-aria-hidden="true" data-backdrop="static">
	<div
		class="modal-dialog">
			...
			<div class="modal-body">
				<form name="credential"
						...
						<div class="col-sm-9">
							<div id="radio-btn-credential-type" class="btn-group btn-group-justified" data-toggle="buttons">
								<c:forEach items="${types}" var="type"
									varStatus="status">
									<label class="btn btn-default ${type==form.type?'active':''}"><spring:message
											code="label.credential.type.${fn:toLowerCase(type)}" />
										<input name="type" type="radio"
										value="${type}" ${type==form.type?'checked="checked"':''} required="required">
									</label>
								</c:forEach>
							</div>
						</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-9">
							<input name="value"
								class="form-control" type="password" value='<spring:message text="${form.value}"/>' required="required"/>
						</div>
					</div>
