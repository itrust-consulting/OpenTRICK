					...
					<tr>
						...
						<c:choose>
								...
								<c:forEach items="${impactTypes}" var="impactType">
									...
									<c:choose>
										<c:when test="${empty impact}">
											<td data-trick-field="${impactName}"   title='<spring:message text="${impactType.acronym}"/>0'
												>0</td>
										</c:when>
										<c:otherwise>
							...
							<c:otherwise>
								...
								<c:if test="${show_uncertainty}"><td title="<fmt:formatNumber value="${assessment.ALEP}" maxFractionDigits="2" /> &euro;"><fmt:formatNumber value="${fct:round(assessment.ALEP*0.001,1)}" /></td></c:if>
							</c:otherwise>
						</c:choose>
						<td   data-trick-field="owner" ><spring:message text="${assessment.owner}" /></td>
