						...
						<tr data-trick-class="Assessment" data-trick-id="${assessment.id}">
							...
							<c:if test="${show_cssf}">
								<c:choose>
									<c:when test="${parameters.containsKey(assessment.impactRep)}">
										<td title='<fmt:formatNumber value="${parameters.get(assessment.impactRep)}" maxFractionDigits="2" /> &euro;'><spring:message
												text="${assessment.impactRep}" /></td>
									</c:when>
									<c:otherwise>
										<td title='<fmt:formatNumber value="${assessment.impactRep}" maxFractionDigits="2" /> &euro;'><c:catch>
								...
								<c:choose>
									<c:when test="${parameters.containsKey(assessment.impactOp)}">
										<td title='<fmt:formatNumber value="${parameters.get(assessment.impactOp)}" maxFractionDigits="2" /> &euro;'><spring:message
												text="${assessment.impactOp}" /></td>
									</c:when>
									<c:otherwise>
										<td title='<fmt:formatNumber value="${assessment.impactOp}" maxFractionDigits="2" /> &euro;'><c:catch>
								...
								<c:choose>
									<c:when test="${parameters.containsKey(assessment.impactLeg)}">
										<td title='<fmt:formatNumber value="${parameters.get(assessment.impactLeg)}" maxFractionDigits="2" /> &euro;'><spring:message
												text="${assessment.impactLeg}" /></td>
									</c:when>
									<c:otherwise>
										<td title='<fmt:formatNumber value="${assessment.impactLeg}" maxFractionDigits="2" /> &euro;'><c:catch>
							...
							<c:choose>
								<c:when test="${parameters.containsKey(assessment.impactFin)}">
									<td  title='<fmt:formatNumber value="${parameters.get(assessment.impactFin)}" maxFractionDigits="2" /> &euro;'><spring:message
											text="${assessment.impactFin}" /></td>
								</c:when>
								<c:otherwise>
									<td title='<fmt:formatNumber value="${assessment.impactFin}" maxFractionDigits="2" /> &euro;'><c:catch>
							...
							<c:choose>
								<c:when test="${parameters.containsKey(assessment.likelihood)}">
									<td
										title='<fmt:formatNumber value="${fct:round(parameters.get(assessment.likelihood),2)}" maxFractionDigits="2" /><spring:message code="label.assessment.likelihood.unit" />'><spring:message
