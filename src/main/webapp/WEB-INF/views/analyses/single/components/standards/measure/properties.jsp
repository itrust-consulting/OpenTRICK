...
<div id="tab_properties" class="tab-pane ${not isAnalysisOnly?'active':''}" style="padding-top: 17px;">
			...
			<tbody>
				<tr id="sliders" style="text-align: center;">
					<td class="warning" data-trick-class="MeasureProperties"><input type="text" class="slider" id="fmeasure" value="${measureForm.properties.getFMeasure()}"
						data-slider-min="0" data-slider-max="10" data-slider-step="1" data-slider-value="${measureForm.properties.getFMeasure()}" name="fmeasure" data-slider-orientation="vertical"
					...
					<td class="warning" data-trick-class="MeasureProperties"><input type="text" class="slider" id="fsectoral" value="${measureForm.properties.getFSectoral()}"
						data-slider-min="0" data-slider-max="4" data-slider-step="1" data-slider-value="${measureForm.properties.getFSectoral()}" name="fsectoral" data-slider-orientation="vertical"
						data-slider-selection="after" data-slider-tooltip="show"></td>
					<c:forEach items="${measureForm.properties.categories.keySet()}" var="category">
						<td class="info" ${not empty cssfExcludes[category]? 'hidden="hidden"' :''} data-trick-class="Category" data-trick-value=<spring:message text="${category}"/>><input
							type="text" class="slider" id="${fn:replace(category,'.','_')}" value="${measureForm.properties.getCategoryValue(category)}" data-slider-min="0" data-slider-max="4"
							data-slider-step="1" data-slider-value="${measureForm.properties.getCategoryValue(category)}" name=<spring:message text="${fn:replace(fn:toLowerCase(category),'.','')}" />
					...
					<c:choose>
						...
						<c:when test="${measureForm.type == 'NORMAL'}">
							<c:forEach items="${measureForm.assetValues}" var="assetValue">
								<td ${not empty hiddenAssetTypes[assetValue.type]? 'hidden="hidden"' :''}><input type="text" class="slider" id='asset_slider_<spring:message text="${assetValue.id}"/>'
									value="${assetValue.value}" data-slider-min="0" data-slider-max="100" data-slider-step="1" data-slider-value="${assetValue.value}"
									name="<spring:message text="${assetValue.id}"/>" data-slider-orientation="vertical" data-slider-selection="after" data-slider-tooltip="show"></td>
							</c:forEach>
						</c:when>
					</c:choose>
				</tr>
