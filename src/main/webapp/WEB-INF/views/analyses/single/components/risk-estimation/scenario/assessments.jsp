...
<div class="col-md-10 trick-ui" id="section_scenario_assessment" data-view='estimation-ui' data-trick-asset-id='-1' data-trick-scenario-id='${scenario.id}'
	data-trick-content='scenario'>
	<div class="page-header tab-content-header">
		<div class="container">
			<div class="row-fluid">
				<h3 role="title">
					<spring:message code="label.assessment.for.scenario" arguments="${scenario.name}" />
				</h3>
			</div>
		</div>
	</div>
		...
		<tbody>
			<c:forEach items="${assessments}" var="assessment">
				<tr data-trick-class="Assessment" data-trick-id="${assessment.id}">
					...
					<c:choose>
						<c:when test="${type == 'QUALITATIVE'}">
							<c:forEach items="${impactTypes}" var="impactType">
								...
								<c:choose>
									...
									<c:otherwise>
										<td data-trick-field="${impactName}" data-trick-field-type="string" class="editable" title='<spring:message text="${impact.variable}"/>' onclick="return editField(this);"><c:choose>
						...
						<c:otherwise>
							...
							<c:choose>
								...
								<c:otherwise>
									<td data-trick-field="likelihood" data-trick-field-type="string" class="editable" onclick="return editField(this);" title='<spring:message text="${likelihood.variable}" />'><fmt:formatNumber
					...
					<td class="editable" onclick="return editField(this);" data-trick-field="comment" data-trick-field-type="string" data-trick-content="text"><spring:message
