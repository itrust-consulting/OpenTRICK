			...
			<div class="form-signin" id="login" style="margin: 0 auto; max-width: 330px; padding: 15px;">
				...
				<form id="login_form" method="post" action="<c:url value='${pageContext.request.contextPath}/j_spring_security_check'/>">
					<div class="form-group">
						<input name="j_username" value="${(!empty (j_username))? j_username : ''}" placeholder="<spring:message code='label.signin.login' />" required="required" class="form-control" />
					</div>
					<div class="form-group">
						<input name="j_password" value="${(!empty (j_password))? j_password : ''}" type="password" class="form-control" placeholder="<spring:message code='label.signin.password' />"
							required="required" />
					</div>
					<div class="checkbox">
