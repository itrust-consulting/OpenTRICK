...
Private Sub UpdateALE(mychart)
    ...
    workSheet.Range("B2:B" & lastNotEmptyRow).NumberFormat = "[>9.99]# ### ### ### ##0KÄ;[>0.01]#0.0KÄ;0KÄ"
...
Private Sub UpdateBudget(mychart)
    ...
    workSheet.Range("F2:H" & workSheet.Range("A" & workSheet.Rows.count).End(myxlUp).row).NumberFormat = "### ### ### ###0KÄ"
...
Private Sub UpdateEvolutionOfProfitability(mychart)
    ...
    workSheet.Range("D2:D" & workSheet.Range("A" & workSheet.Rows.count).End(myxlUp).row).NumberFormat = "### ### ### ###0KÄ"
...
Sub UpdateGraphics()
Attribute UpdateGraphics.VB_Description = "Applique le style Normal et convertit les titres s√©lectionn√©s en corps de texte."
Attribute UpdateGraphics.VB_ProcData.VB_Invoke_Func = "TemplateProject.NewMacros.AbaisserEnCorpsDeTexte"
'Update chart
'Mise ‡† jour des graphs
'
    Call Initialisation
