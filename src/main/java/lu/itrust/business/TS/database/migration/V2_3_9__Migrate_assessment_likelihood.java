...
public class V2_3_9__Migrate_assessment_likelihood extends TrickServiceDataBaseMigration {
	...
	private List<DynamicParameter> loadDynamicParameters(JdbcTemplate template, Integer analysisId) {
		return template.query("Select `dtAcronym`,`dtDescription`, `dtValue` From DynamicParameter where fiAnalysis = ?", new Object[] { analysisId },
				(row, cout) -> new DynamicParameter(row.getString("dtAcronym"), row.getString("dtDescription"), row.getDouble("dtValue")));
	}
