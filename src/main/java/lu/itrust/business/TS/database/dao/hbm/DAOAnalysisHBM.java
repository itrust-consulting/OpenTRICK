...
public class DAOAnalysisHBM extends DAOHibernate implements DAOAnalysis {
	...
	public Analysis getDefaultProfile() {
		return (Analysis) getSession().createQuery("Select analysis From Analysis analysis where analysis.defaultProfile = true and analysis.profile = true").uniqueResultOptional().orElse(null);
	}
	...
	public boolean exists(String identifier, String version) {
		String query = "Select count(*) > 0 From Analysis analysis where analysis.identifier = :identifier and analysis.version = :version";
		return (boolean) getSession().createQuery(query).setParameter("identifier", identifier).setParameter("version", version).getSingleResult();
	}
	...
	public boolean isProfile(Integer analysisid) {
		...
		return (boolean) getSession().createQuery(query).setParameter("identifier", analysisid).uniqueResultOptional().orElse(false);
	...
	public boolean hasData(Integer idAnalysis) {
		...
		return (boolean) getSession().createQuery(query).setParameter("identifier", idAnalysis).uniqueResultOptional().orElse(false);
	...
	public List<AnalysisBaseInfo> getBaseInfoByCustmerIdAndUsernamerAndIdentifierAndNotEmpty(Integer id, String username, String identifier, List<AnalysisRight> rights) {
		...
		query += "analysis.customer.id = :customer order by analysis.creationDate desc, analysis.identifier asc, analysis.version desc";
		return (List<AnalysisBaseInfo>) getSession().createQuery(query).setParameter("username", username).setParameterList("rights", rights).setParameter("identifier", identifier)
	...
	public int getDefaultProfileId() {
		return  (int) getSession().createQuery("Select analysis.id From Analysis analysis where analysis.defaultProfile = true and analysis.profile = true")
	...
	public List<Analysis> getFromUserNameAndNotEmpty(String userName, List<AnalysisRight> rights) {
		return getSession()
				...
				.setParameter("username", userName).setParameterList("rights", rights).getResultList();
	}
	...
	public AnalysisType getAnalysisTypeById(int idAnalysis) {
		return (AnalysisType) getSession().createQuery("Select type From Analysis where id = :idAnalysis").setParameter("idAnalysis", idAnalysis).uniqueResultOptional().orElse(null);
	}
}
