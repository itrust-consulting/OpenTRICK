...
public class AssessmentAndRiskProfileManager {
	...
	public void UpdateAssessment(Analysis analysis) throws Exception {
		...
		Map<String, ExtendedParameter> parameters = analysis.mapExtendedParameterByAcronym();
		if (analysis.isCssf() || !analysis.getRiskProfiles().isEmpty()) {
			...
			for (Asset asset : analysis.getAssets()) {
				for (Scenario scenario : analysis.getScenarios()) {
					...
					if (scenario.hasInfluenceOnAsset(asset.getAssetType())) {
						if (assessment == null)
							analysis.getAssessments().add(ComputeAlE(new Assessment(asset, scenario), parameters));
						if (riskProfile == null)
	...
	public static Assessment ComputeAlE(Assessment assessment, Map<String, ExtendedParameter> parameters) {
