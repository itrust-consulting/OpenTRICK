...
public class ControllerAnalysisStandard {
	...
	public @ResponseBody String addStandard(@PathVariable int idStandard, HttpSession session, Principal principal,
		...
		try {
			...
			if (analysis.getAnalysisStandards().stream()
					.anyMatch(a -> a.getStandard().getLabel().equalsIgnoreCase(standard.getLabel())))
				return JsonMessage.Error(messageSource.getMessage("error.analysis.add.standard.duplicate", null,
	...
	public @ResponseBody Object computeEfficience(@RequestBody List<String> chapters, HttpSession session,
		...
		return MeasureManager.ComputeMaturiyEfficiencyRate(measures, maturities, loadMaturityParameters(idAnalysis),
				false, new ValueFactory(serviceDynamicParameter.findByAnalysisId(idAnalysis)));
	}
	...
	public @ResponseBody Object createStandardForm(@RequestBody String value, HttpSession session, Principal principal,
			Model model, RedirectAttributes attributes, Locale locale) throws Exception {
		...
		try {
			...
			if (standard.getId() < 1) {
				if (analysis.getAnalysisStandards().stream()
						.anyMatch(analysisStandard -> analysisStandard.getStandard().hasSameName(standard)))
					return JsonMessage.Field("label",
							messageSource.getMessage("error.analysis.standard_exist_in_analysis", null,
	...
	public @ResponseBody String deleteMeasureDescription(@PathVariable("idStandard") int idStandard,
			@PathVariable("idMeasure") int idMeasure, Locale locale, Principal principal, HttpSession session) {
		try {
		...
		try {
			...
			if (measure == null)
				throw new TrickException("error.measure.not_found", "Measure cannot be found");
					...
					|| measure.getAnalysisStandard().getStandard().isAnalysisOnly()))
				throw new TrickException("error.action.not_authorise", "Action does not authorised");
	...
	public @ResponseBody String generateTickets(@RequestBody TicketingForm form, Principal principal,
		...
		Worker worker = new WorkerGenerateTickets((Integer) session.getAttribute(Constant.SELECTED_ANALYSIS), null,
				form, serviceTaskFeedback, workersPoolManager, sessionFactory);
	...
	public String getSingleMeasure(@PathVariable int elementID, Model model, HttpSession session, Principal principal)
		...
		model.addAttribute("isLinkedToProject",
				serviceAnalysis.hasProject(idAnalysis) && loadUserSettings(principal, model, null));
	...
	public String linkTickets(@RequestBody List<Integer> measureIds, Model model, Principal principal,
		...
		try {
			...
			if (analysis.hasProject()) {
				...
				List<String> excludes = analysis.getAnalysisStandards().stream()
				...
				if (measureIds.size() > 5) {
					Map<Integer, Integer> contains = measureIds.stream()
							.collect(Collectors.toMap(Function.identity(), Function.identity()));
					measures = analysis.getAnalysisStandards().stream()
							.flatMap(listMeasures -> listMeasures.getMeasures().stream())
							.filter(measure -> contains.containsKey(measure.getId())
									&& StringUtils.isEmpty(measure.getTicket()))
							.collect(Collectors.toList());
				} else {
					measures = analysis.getAnalysisStandards().stream()
							.flatMap(listMeasures -> listMeasures.getMeasures().stream())
							.filter(measure -> StringUtils.isEmpty(measure.getTicket())
	...
	public @ResponseBody String loadDescription(@PathVariable("idMeasure") int idMeasure,
			@PathVariable("langue") String langue, Principal principal, HttpSession session, Locale locale) {
	...
	public @ResponseBody Object loadMeasureByStandard(@RequestParam("idStandard") Integer idStandard,
		...
		return serviceMeasure
				...
				.sorted(new MeasureComparator()).map(measure -> {
					...
					measureForm
							.setImplementationRate((int) measure.getImplementationRateValue(Collections.emptyList()));
	...
	public String loadSOA(HttpSession session, Principal principal, Model model) throws Exception {
		...
		model.addAttribute("soas",
				serviceAnalysisStandard.findBySOAEnabledAndAnalysisId(true, idAnalysis).stream()
						.sorted((e1, e2) -> NaturalOrderComparator.compareTo(e1.getStandard().getLabel(),
								e2.getStandard().getLabel()))
	...
	public String loadTickets(@RequestBody List<Integer> measureIds, @RequestParam(name = "startIndex") int startIndex,
			Model model, Principal principal, HttpSession session, RedirectAttributes attributes, Locale locale) {
		Client client = null;
		try {
			if (!loadUserSettings(principal, model, null))
				throw new ResourceNotFoundException();
	...
	public String manageSOA(HttpSession session, Principal principal, Model model, RedirectAttributes attributes,
			Locale locale) throws Exception {
		model.addAttribute("analysisStandards", serviceAnalysisStandard.findByAndAnalysisIdAndTypeIn(
				(Integer) session.getAttribute(Constant.SELECTED_ANALYSIS), NormalStandard.class, AssetStandard.class));
	...
	public String measureForm(@PathVariable("idMeasure") int idMeasure, Locale locale, Model model, Principal principal,
		...
		try {
			...
			MeasureDescriptionText measureDescriptionText = measureDescription
					.getMeasureDescriptionTextByAlpha2(locale.getLanguage());
			...
			if (measureDescriptionText != null) {
				...
				if (!(otherMeasureDescriptionText == null
						|| StringUtils.isEmpty(otherMeasureDescriptionText.getDescription())))
	...
	public @ResponseBody Map<String, Object> measuresave(@RequestBody MeasureForm measureForm, Model model,
		...
		try {
			...
			if (measureForm.getId() > 0) {
				...
				if (measure == null)
					errors.put("measure", messageSource.getMessage("error.measure.not_found", null,
			...
			if (analysisStandard.getStandard().isAnalysisOnly()) {
				...
				if (!errors.isEmpty())
					return result;
				if (!update(measure, measureForm, idAnalysis, type, serviceLanguage.getFromAnalysis(idAnalysis), locale,
						errors).isEmpty())
					return result;
			...
			if (analysis.hasProject()) {
				...
				if (measures.size() > 5) {
					Map<Integer, Integer> contains = measures.stream()
							.collect(Collectors.toMap(Function.identity(), Function.identity()));
					keyIssues = analysis.getAnalysisStandards().stream()
							.flatMap(listMeasures -> listMeasures.getMeasures().stream())
							.filter(measure -> contains.containsKey(measure.getId())
									&& !StringUtils.isEmpty(measure.getTicket()))
				...
				Map<String, TicketingTask> taskMap = client
				...
				if (maturityStandard.getStandard().isComputable())
					model.addAttribute("effectImpl27002",
							MeasureManager.ComputeMaturiyEfficiencyRate(analysisStandard.getMeasures(),
									maturityStandard.getMeasures(), loadMaturityParameters(idAnalysis), true, factory));
	...
	public String synchroniseWithTicketingSystem(@RequestBody List<Integer> ids, Model model, Principal principal,
		...
		try {
			...
			if (analysis.hasProject()) {
				...
				if (ids.size() > 5) {
					Map<Integer, Integer> contains = ids.stream()
							.collect(Collectors.toMap(Function.identity(), Function.identity()));
					measuresMap = analysis.getAnalysisStandards().stream()
							.flatMap(listMeasures -> listMeasures.getMeasures().stream())
							.filter(measure -> contains.containsKey(measure.getId())
									&& !StringUtils.isEmpty(measure.getTicket()))
				...
				Map<String, TicketingTask> tasks = client.findByIdsAndProjectId(analysis.getProject(), keyIssues)
						.stream().collect(Collectors.toMap(task -> task.getId(), Function.identity()));
				List<? extends IParameter> parameters = analysis
	...
	private boolean loadUserSettings(@Nonnull Principal principal, @Nullable Model model, @Nullable User user) {
		...
		try {
			...
			TSSetting name = serviceTSSetting.get(TSSettingName.TICKETING_SYSTEM_NAME),
					url = serviceTSSetting.get(TSSettingName.TICKETING_SYSTEM_URL);
			String username = user.getSetting(Constant.USER_TICKETING_SYSTEM_USERNAME),
					password = user.getSetting(Constant.USER_TICKETING_SYSTEM_PASSWORD);
			allowedTicketing = !(name == null || url == null || StringUtils.isEmpty(name.getValue())
					|| StringUtils.isEmpty(url.getValue()) || StringUtils.isEmpty(username)
	...
	private Map<String, Object> update(Measure measure, MeasureForm measureForm, Integer idAnalysis, AnalysisType type,
			Language language, Locale locale, Map<String, Object> errors) throws Exception {
		if (errors == null)
			errors = new LinkedHashMap<String, Object>();
		MeasureDescription description = measure.getMeasureDescription();
		if (description == null) {
			if (serviceMeasureDescription.existsForMeasureByReferenceAndAnalysisStandardId(measureForm.getReference(),
					measure.getAnalysisStandard().getId())) {
				errors.put("reference",
						messageSource.getMessage("error.measure_description.reference.duplicated",
								new String[] { measure.getAnalysisStandard().getStandard().getLabel() },
	...
	private void updateMeasureCost(double externalSetupValue, double internalSetupValue, double lifetimeDefault,
			Analysis analysis) {
		analysis.getAnalysisStandards().stream()
