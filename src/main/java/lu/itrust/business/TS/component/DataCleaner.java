...
public class DataCleaner {
	...
	private void cleanUpReport(int pageSize, int maxSize) {
		...
		final int pageCount = Math.max((int) Math.ceil((double) Math.min(daoWordReport.countByCreatedBefore(deleteDate), maxSize) / (double) pageSize), 1);
		for (int i = 1; i <= pageCount; i++) {
			...
			for (WordReport report : reports) {
				...
				TrickLogManager.Persist(LogLevel.INFO, LogType.SYSTEM, "log.system.data.cleaner.report",
						String.format("Type: report, analysis: %s,name: %s, version: %s, exported: %s, owner: %s", report.getIdentifier(), report.getLabel(), report.getVersion(),
								report.getCreated(), report.getUser().getLogin()),
						"Data cleaner", LogAction.DELETE, report.getIdentifier(), report.getLabel(), report.getVersion(), String.valueOf(report.getCreated()),
	...
	private void cleanUpSQLITE(int pageSize, int maxSize) {
		...
		final int pageCount = Math.max((int) Math.ceil((double) Math.min(daoUserSqLite.countByCreatedBefore(deleteDate), maxSize) / (double) pageSize), 1);
		for (int i = 1; i <= pageCount; i++) {
			...
			for (UserSQLite sqlite : sqLites) {
				...
				TrickLogManager.Persist(LogLevel.INFO, LogType.SYSTEM, "log.system.data.cleaner.sqlite",
						String.format("Type: database, analysis: %s,name: %s, version: %s, exported: %s, owner: %s", sqlite.getIdentifier(), sqlite.getLabel(), sqlite.getVersion(),
								sqlite.getCreated(), sqlite.getUser().getLogin()),
						"Data cleaner", LogAction.DELETE, sqlite.getIdentifier(), sqlite.getLabel(), sqlite.getVersion(), String.valueOf(sqlite.getCreated()),
