...
public class Docx4jAssessmentBuilder extends Docx4jBuilder {
	...
	private boolean buildQualitative(Docx4jData data) {
		...
		if (paragraphOrigin != null) {
			...
			final List<ScaleType> scaleTypes = analysis.getImpactParameters().stream().map(p -> p.getType()).distinct()
					...
					.sorted((s1, s2) -> NaturalOrderComparator.compareTo(s1.getShortName(language), s2.getShortName(language))).collect(Collectors.toList());
			...
			assessementsByAsset.keySet().forEach(asset -> {
				...
				exporter.setCellText((Tc) header.getContent().get(hColIndex++), exporter.getMessage("report.assessment.scenarios", null, "Scenarios"), alignmentLeft);
				for (ScaleType scaleType : scaleTypes)
					exporter.setCellText((Tc) header.getContent().get(hColIndex++), scaleType.getShortName(language), alignmentCenter);

				exporter.setCellText((Tc) header.getContent().get(hColIndex++), exporter.getMessage("report.assessment.probability", null, "P."), alignmentCenter);
				exporter.setCellText((Tc) header.getContent().get(hColIndex++), exporter.getMessage("report.assessment.owner", null, "Owner"));
				exporter.setCellText((Tc) header.getContent().get(hColIndex++), exporter.getMessage("report.assessment.comment", null, "Comment"));

				for (Assessment assessment : assessments) {
					final Tr row = (Tr) table.getContent().get(++rawLength);
					int colIndex = 0;
					exporter.setCellText((Tc) row.getContent().get(colIndex++), assessment.getScenario().getName(), alignmentLeft);
					for (ScaleType scaleType : scaleTypes) {
				...
				contents.add(table);
				contents.add(exporter.addTableCaption(exporter.getMessage("report.assessment.table.caption", new Object[] { asset.getName() },
	...
	private boolean buildQuantitative(Docx4jData data) {
		...
		if (paragraphOrigin != null) {
			...
			final boolean mixted = analysis.isQualitative() && (boolean) analysis.findSetting(AnalysisSetting.ALLOW_QUALITATIVE_IN_QUANTITATIVE_REPORT);
			final List<ScaleType> scaleTypes = analysis.getImpactParameters().stream().filter(p -> mixted && !p.getTypeName().equals(Constant.DEFAULT_IMPACT_NAME))
			...
			exporter.setText(paraph, exporter.getMessage("report.assessment.total_ale.assets", null, "Total ALE for all assets"));
			...
			alesmap.values().stream().sorted(new AssetComparatorByALE()).forEach(ale -> {
				...
				final Tbl table = exporter.createTable("TableTSAssessment", assessmentsofasset.size() + 1, 6 + scaleTypes.size());
				P paragraph = exporter.getFactory().createP();
				exporter.setText(paragraph, ale.getAssetName());
				exporter.setStyle(paragraph, "TSEstimationTitle");
				contents.add(paragraph);
				paragraph = exporter.getFactory().createP();
				exporter.setText(paragraph, exporter.getMessage("report.assessment.total.ale.for.asset", null, "Total ALE of asset"));
				exporter.addTab(paragraph);
				exporter.setText(paragraph, String.format("%s k€", exporter.getKiloNumberFormat().format(ale.getValue() * 0.001)), true);
				exporter.setStyle(paragraph, "TSAssessmentTotalALE");
				contents.add(paragraph);
				int rowIndex = 0, colIndex = 0;
				Tr row = (Tr) table.getContent().get(rowIndex++);
				exporter.setCellText((Tc) row.getContent().get(colIndex++), exporter.getMessage("report.assessment.scenarios", null, "Scenarios"), alignmentLeft);
				exporter.setCellText((Tc) row.getContent().get(colIndex++), exporter.getMessage("report.assessment.impact.financial", null, "Fin."), alignmentCenter);
				for (ScaleType impact : scaleTypes)
					exporter.setCellText((Tc) row.getContent().get(colIndex++), impact.getShortName(exporter.getLocale().getLanguage()), alignmentCenter);
				exporter.setCellText((Tc) row.getContent().get(colIndex++), exporter.getMessage("report.assessment.probability", null, "P."), alignmentCenter);
				exporter.setCellText((Tc) row.getContent().get(colIndex++), exporter.getMessage("report.assessment.ale", null, "ALE(k€/y)"));
				exporter.setCellText((Tc) row.getContent().get(colIndex++), exporter.getMessage("report.assessment.owner", null, "Owner"));
				exporter.setCellText((Tc) row.getContent().get(colIndex++), exporter.getMessage("report.assessment.comment", null, "Comment"));
				for (Assessment assessment : assessmentsofasset) {
					...
					exporter.addCellNumber((Tc) row.getContent().get(colIndex++),
							exporter.getKiloNumberFormat().format(assessment.getImpactValue(Constant.DEFAULT_IMPACT_NAME) * 0.001));
				...
				contents.add(table);
				contents.add(exporter.addTableCaption(exporter.getMessage("report.assessment.table.caption", new Object[] { ale.getAssetName() },
