...
public class Docx4jStandardBuilder extends Docx4jBuilder {
	...
	private static final String[] TS_2700X_CHART_IDS = { "chartcompliance27001", "chartcompliance27002", TS_QL_CHARTCOMPLIANCE27001, TS_QT_CHARTCOMPLIANCE27001,
			...
			if (!analysisStandards.isEmpty()) {
				...
				if (chart2700x != null) {
					...
					for (Object[] objects : charts)
						buildComplianceChart((Chart) objects[0], (ActionPlanMode) objects[1], (String) objects[2], exporter, phases);
				}
			}
		}
		return true;
	...
	private void buildComplianceChart(Part part, ActionPlanMode mode, String standard, Docx4jReportImpl exporter, final List<Phase> phases) throws Exception {
		...
		if (standard.equals(STANDARD_27001) || standard.equals(STANDARD_27002))
			r.setT(exporter.getMessage("report.compliance.iso", new Object[] { standard }, "Compliance ISO " + standard));
		else {
			r.setT(exporter.getMessage("report.compliance.custom", new Object[] { standard }, "Compliance " + standard));
		...
		final Map<Integer, Boolean> actionPlanMeasures = exporter.getAnalysis().findIdMeasuresImplementedByActionPlanType(mode);
				...
				for (Measure measure : analysisStandard.getMeasures()) {
					...
					if (!measure.getMeasureDescription().isComputable()) {
						...
						exporter.setCellText((Tc) row.getContent().get(2),
								exporter.getMessage("label.measure.status." + measure.getStatus().toLowerCase(), null, measure.getStatus()));
						exporter.addCellNumber((Tc) row.getContent().get(3), exporter.getNumberFormat().format(measure.getImplementationRateValue(exporter.getValueFactory())));
						exporter.addCellNumber((Tc) row.getContent().get(4), exporter.getKiloNumberFormat().format(measure.getInternalWL()));
						exporter.addCellNumber((Tc) row.getContent().get(5), exporter.getKiloNumberFormat().format(measure.getExternalWL()));
						exporter.addCellNumber((Tc) row.getContent().get(6), exporter.getKiloNumberFormat().format(measure.getInvestment() * 0.001));
						exporter.addCellNumber((Tc) row.getContent().get(7), exporter.getNumberFormat().format(measure.getLifetime()));
						exporter.addCellNumber((Tc) row.getContent().get(8), exporter.getKiloNumberFormat().format(measure.getInternalMaintenance()));
						exporter.addCellNumber((Tc) row.getContent().get(9), exporter.getKiloNumberFormat().format(measure.getExternalMaintenance()));
						exporter.addCellNumber((Tc) row.getContent().get(10), exporter.getNumberFormat().format(measure.getRecurrentInvestment() * 0.001));
						exporter.addCellNumber((Tc) row.getContent().get(11), exporter.getNumberFormat().format(measure.getCost() * 0.001));
						exporter.addCellParagraph((Tc) row.getContent().get(12), measure.getPhase().getNumber() + "");
						exporter.addCellParagraph((Tc) row.getContent().get(13), measure.getResponsible());
						exporter.addCellParagraph((Tc) row.getContent().get(14), measure.getToDo());
						if (MEASURE_STATUS_NOT_APPLICABLE.equalsIgnoreCase(measure.getStatus()) || measure.getImplementationRateValue(exporter.getValueFactory()) >= 100) {
							...
							setColor((Tc) row.getContent().get(11), measure.getCost() == 0 ? exporter.getZeroCostColor() : exporter.getDefaultColor());
						}
					}
	...
	private Chart find27001Part(Docx4jReportImpl exporter) throws InvalidFormatException, XPathBinderAssociationIsPartialException, JAXBException {
