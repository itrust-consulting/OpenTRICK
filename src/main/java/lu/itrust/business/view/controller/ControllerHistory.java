...
public class ControllerHistory {
	...
	public String display(@PathVariable("analysisId") Integer analysisId,
	...
	public String displayHistory(@PathVariable("historyId") Integer historyId,
		...
		if (history == null) {
			String msg = messageSource.getMessage("errors.history.notexist",
					null, "History does not exist", locale);
	 ...
	 * 
	 * */
	@RequestMapping("/Add/Analysis/{analysisId}")
	public String addHistory(@PathVariable("analysisId") Integer analysisId,
	...
	@RequestMapping("/Save/Analysis/{analysisId}")
	public String save(@ModelAttribute @Valid History history,
			BindingResult result, @PathVariable Integer analysisId,
		...
		for (String version : serviceHistory.findVersionByAnalysis(analysisId)) {
			if (History.VersionComparator(history.getVersion(), version) != 1) {
				result.rejectValue("version",
						"error.history.version.less_current",
						"Version of History entry must be greater than last Version of Analysis!");
				return "analysis/components/widgets/historyForm";
			}
		}
	...
	public String createHistory(@PathVariable("analysisId") Integer analysisId,
			@ModelAttribute("history") @Valid History history,
			BindingResult result, RedirectAttributes redirectAttributes,
			Locale locale) throws Exception {
		if (!this.serviceHistory
				.versionExists(analysisId, history.getVersion())) {
			try {
				this.serviceHistory.save(analysisId, history);
			} catch (Exception e) {
				String msg = messageSource.getMessage(
						"errors.history.wrongversion", null, e.getMessage(),
						locale);
				redirectAttributes.addFlashAttribute("errors", msg);
				return "redirect:../History/Add";
			}
			String msg = messageSource.getMessage("success.history.added",
					null, "History sucessfully added!", locale);
			redirectAttributes.addFlashAttribute("errors", msg);
			return "redirect:../History/Display";
		} else {
			String msg = messageSource.getMessage(
					"errors.history.versionexists", null,
					"History with this version already exists!", locale);
			redirectAttributes.addFlashAttribute("errors", msg);
			return "redirect:../History/Add";
		}
	...
	public String editHistory(@PathVariable("analysisId") Integer analysisId,
			@PathVariable("historyId") Integer historyId, HttpSession session,
	...
	public String updateCustomer(
		...
		if (history == null || history.getId() != historyId) {
			String msg = messageSource.getMessage(
					"errors.history.update.notrecognized", null,
					"History not recognized", locale);
			redirectAttributes.addFlashAttribute("errors", msg);
		} else {
			try {
				serviceHistory.saveOrUpdate(history);
				String msg = messageSource.getMessage(
						"success.history.update.success", null,
						"History had been updated!", locale);
				redirectAttributes.addFlashAttribute("success", msg);
			} catch (Exception e) {
				String msg = messageSource.getMessage(
						"errors.history.update.fail", null,
						"History update failed!", locale);
				redirectAttributes.addFlashAttribute("errors", msg);
			}
		}
		return "redirect:/Analysis/" + analysisId + "/History/Display";
	...
	public String deleteCustomer(@PathVariable("historyId") Integer historyId)
			throws Exception {
