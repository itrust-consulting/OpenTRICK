...
public class StringTokenizer implements Tokenizer {
	 ...
	 * Note that according to http://stackoverflow.com/questions/8894258/fastest-way-to-iterate-over-all-the-chars-in-a-string
	 * the fastest method to iterate over small strings (up to 256 characters) is by using .charAt().
	 */
	private final String expressionString;
	/**
	 * Points to the next character in the string which has not been processed yet.
	 */
	private int pointer = 0;
	
	@Override
	public Token read() throws InvalidExpressionException {
		// Read characters unless we find a valid token ('return' inside loop body) or the string ends
	...
	/**
	 * Reads a floating point number from the expression at the current position.
	 * Note that scientific notation (e.g. 1E-4) is not supported. The fractional part must be introduced by a period ('.') only.
	 * PRE-CONDITION: The pointer must point to the first character of a number.
	...
	private Token readNumber() throws InvalidExpressionException {
		...
		// Continuously read characters until end of expression has been reached,
	 ...
	 * PRE-CONDITION: The pointer must point to the first character of a variable.
	...
	private Token readVariable() {
		...
		for (; this.pointer < this.expressionString.length(); this.pointer++) {
			...
			// NB: Although variables are not allowed to begin with a digit, we know that this is not
			// the case because of the PRE-CONDITION of this method. In other words, we do not have
			// to treat the first character of a variable apart.

			// Valid characters are digits, letters and underscores;
			// if we read something else, our token ends here and we can return it.
			if (!Character.isDigit(nextChar) && !Character.isLetter(nextChar) && nextChar != '_')
				break;
		}

		// We know the variable is NOT empty because of the PRE-CONDITION of this method
