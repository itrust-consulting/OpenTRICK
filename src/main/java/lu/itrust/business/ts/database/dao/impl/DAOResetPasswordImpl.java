 ...
 */
@Repository
public class DAOResetPasswordImpl extends DAOHibernate implements DAOResetPassword {
	
	/**
	 * 
	 */
	public DAOResetPasswordImpl() {
	}

	/**
	 * @param session
	 */
	public DAOResetPasswordImpl(Session session) {
		super(session);
	}
	...
	public ResetPassword get(User user) {
		return (ResetPassword) createQueryWithCache("From ResetPassword where user = :user").setParameter("user", user).uniqueResultOptional().orElse(null);
	}
	...
	public ResetPassword get(String keyControl) {
		return (ResetPassword) createQueryWithCache("From ResetPassword where keyControl = :keyControl").setParameter("keyControl", keyControl).uniqueResultOptional().orElse(null);
	}
	...
	public List<ResetPassword> getAll() {
		return createQueryWithCache("From ResetPassword").getResultList();
	}
	...
	public List<ResetPassword> getAll(int page, int size) {
		return createQueryWithCache("From ResetPassword").setMaxResults(size).setFirstResult((page-1)*size).getResultList();
	}
