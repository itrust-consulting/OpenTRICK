 ...
 */
@Repository
public class DAOTicketingSystemHBM extends DAOHibernate implements DAOTicketingSystem {

	/**
	 * 
	 */
	public DAOTicketingSystemHBM() {
	}

	/**
	 * @param session
	 */
	public DAOTicketingSystemHBM(Session session) {
		super(session);
	}
	...
	@Override
	public long count() {
		return getSession().createQuery("Select count(*) From TicketingSystem", Long.class).uniqueResult();
	}
	...
	public void delete(Long id) {
		getSession().createQuery("Delete From TicketingSystem where id = :id").setParameter("id", id).executeUpdate();
	}
	...
	public void deleteAll() {
		getSession().createQuery("Delete From TicketingSystem").executeUpdate();
	}
	...
	public boolean exists(Long id) {
		return getSession().createQuery("Select count(*)> 0 From TicketingSystem where id = :id", Boolean.class).setParameter("id", id).uniqueResult();
	}
	...
	public List<TicketingSystem> findAll() {
		return getSession().createQuery("From TicketingSystem",TicketingSystem.class ).list();
	}
	...
	public List<TicketingSystem> findAll(List<Long> ids) {
		return getSession().createQuery("From TicketingSystem where id in :ids", TicketingSystem.class).setParameterList("ids", ids).list();
	}
