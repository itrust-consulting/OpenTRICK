 ...
 */
@Repository
public class DAOMeasureImpl extends DAOHibernate implements DAOMeasure {
	 ...
	 * @param session
	 */
	public DAOMeasureImpl(Session session) {
		super(session);
	}
	...
	public Measure getFromAnalysisById(Integer idAnalysis, Integer id) {
		...
		return (Measure) createQueryWithCache(query).setParameter("analysis", idAnalysis).setParameter("id", id).uniqueResultOptional().orElse(null);
	}
	...
	public boolean belongsToAnalysis(Integer analysisId, Integer measureId) {
		...
		return (boolean) createQueryWithCache(query).setParameter("analysisId", analysisId).setParameter("measureId", measureId).getSingleResult();
	}
	...
	public List<Measure> getAll() {
		return (List<Measure>) createQueryWithCache("From Measure").getResultList();
	}
	...
	public List<Measure> getAllFromAnalysis(Integer idAnalysis) {
		...
		return createQueryWithCache(query).setParameter("analysis", idAnalysis).getResultList();
	}
	...
	public List<Measure> getAllFromAnalysisAndStandard(Integer idAnalysis, Integer idStandard) {
		...
		return createQueryWithCache(query).setParameter("analysis", idAnalysis).setParameter("idStandard", idStandard).getResultList();
	}
	...
	public List<Measure> getAllFromAnalysisAndStandard(Integer idAnalysis, String standard) {
		...
		query += ":analysis order by measure.id";
		return createQueryWithCache(query).setParameter("analysis", idAnalysis).setParameter("standard", standard).getResultList();
	}
	...
	public List<Measure> getAllNotMaturityMeasuresFromAnalysisByMeasureIdList(Integer idAnalysis, List<Integer> measures) {
		...
		return createQueryWithCache(query).setParameter("analysis", idAnalysis).setParameterList("measures", measures).getResultList();
	}
	...
	public Map<String, Measure> mappingAllFromAnalysisAndStandard(Integer idAnalysis, Integer idStandard) {
		return (Map<String, Measure>) createQueryWithCache(
	...
	public int countNormalMeasure() {
		return (int) createQueryWithCache("Select count(*) From NormalMeasure").uniqueResultOptional().orElse(0);
	}
	...
	public List<NormalMeasure> getAllNormalMeasure(int pageIndex, int pageSize) {
		return createQueryWithCache("From NormalMeasure").setFirstResult((pageIndex - 1) * pageSize).setMaxResults(pageSize).getResultList();
	}
	...
	public Measure getFromAnalysisAndStandardAndReference(Integer idAnalysis, Integer idStandard, String reference) {
		...
		return (Measure) createQueryWithCache(query).setParameter("analysis", idAnalysis).setParameter("idStandard", idStandard).setParameter("reference", reference)
	...
	public Measure getByAnalysisAndStandardAndReference(Integer idAnalysis, String standard, String reference) {
		return (Measure) createQueryWithCache(
	...
	public List<Measure> getByAnalysisIdStandardAndChapters(Integer idAnalysis, String standard, List<String> chapters) {
		...
		if (!subSql.isEmpty())
			request += " and (" + subSql + ")";
		Query<Measure> query = createQueryWithCache(request).setParameter("idAnalysis", idAnalysis).setParameter("standard", standard);
		for (int i = 0; i < chapters.size(); i++)
