 ...
 */
@Repository
public class DAOUserImpl extends DAOHibernate implements DAOUser {
	...
	public User get(String login) {
		return createQueryWithCache("From User where login = :login", User.class).setParameter("login", login)
	...
	public User get(String login, String password) {
		return createQueryWithCache("From User where login = :login and password = :password", User.class)
	...
	public boolean noUsers() {
		return createQueryWithCache("Select count(*) = 0 From User", Boolean.class).getSingleResult();
	}
	...
	public List<User> getAll() {
		return createQueryWithCache("From User order by firstName", User.class).getResultList();
	}
	...
	public List<User> getAllByFirstName(String name) {
		return createQueryWithCache("From User where firstName = :name", User.class).setParameter("name", name)
				.getResultList();
	}
	...
	public List<User> getAllByCountry(String name) {
		return createQueryWithCache("From User where country = :country", User.class).setParameter("country", name)
				.getResultList();
	}
	...
	public List<User> getAllAdministrators() {
		return createQueryWithCache("SELECT user From User as user join user.roles as role where role.type = :role",
	...
	public User getByEmail(String email) {
		return (User) createQueryWithCache("From User where email = :email", User.class)
				.setParameter("email", email).uniqueResultOptional().orElse(null);
	}

	@Override
	public List<User> getAllOthers(Collection<User> users) {
		return createQueryWithCache("From User user where user not in :users", User.class)
				.setParameterList("users", users).getResultList();
	}

	@Override
	public List<User> getAllOthers(User user) {
		return createQueryWithCache("From User user where user <> :user", User.class).setParameter("user", user)
				.getResultList();
	}

	@Override
	public boolean existByUsername(String username) {
		return (boolean) createQueryWithCache("Select count(*)> 0 From User where login = :username", Boolean.class)
				.setParameter("username", username).getSingleResult();
	}

	@Override
	public boolean existByEmail(String email) {
		return (boolean) createQueryWithCache("Select count(*)> 0 From User where email = :email", Boolean.class)
	...
	public List<User> getAll(Collection<Integer> ids) {
		return ids.isEmpty() ? Collections.emptyList()
				: createQueryWithCache("From User where id in :ids", User.class).setParameterList("ids", ids)
						.getResultList();
	}

	@Override
	public String findLocaleByUsername(String username) {
		return createQueryWithCache("Select locale From User where login = :username", String.class)
	...
	public String findUsernameById(Integer id) {
		return createQueryWithCache("Select login From User where id = :id", String.class).setParameter("id", id)
				.uniqueResult();
	}
	...
	public String findUsernameByEmail(String email) {
		return createQueryWithCache("Select login From User where email = :email", String.class).setParameter("email", email)
				.uniqueResult();
	}
}