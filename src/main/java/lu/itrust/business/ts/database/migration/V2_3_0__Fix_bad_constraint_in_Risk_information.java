...
public class V2_3_0__Fix_bad_constraint_in_Risk_information extends TrickServiceDataBaseMigration {
	...
	public void migrate(JdbcTemplate jdbcTemplate) throws Exception {
		ResultSet foreignKeys = jdbcTemplate.getDataSource().getConnection().getMetaData().getImportedKeys(getSchema(),null,
				 "RiskInformation");
		while (foreignKeys.next())
			deleteForeignKeys(jdbcTemplate, "RiskInformation", foreignKeys.getString("FK_NAME"));

		ResultSet uniqueKeys = jdbcTemplate.getDataSource().getConnection().getMetaData().getIndexInfo(getSchema(),null,
