 ...
 */
@Repository
public class DAOAssetImpl extends DAOHibernate implements DAOAsset {
	 ...
	 * @param session
	 */
	public DAOAssetImpl(Session session) {
		super(session);
	}
	...
	public boolean belongsToAnalysis(Integer analysisId, Integer assetId) {
		...
		return (boolean) createQueryWithCache(query).setParameter("analysisid", analysisId).setParameter("assetid", assetId).getSingleResult();
	}
	...
	public Asset getFromAnalysisByName(Integer analysisId, String name) {
		...
		return (Asset) createQueryWithCache(query).setParameter("analysisid", analysisId).setParameter("name", name).uniqueResultOptional().orElse(null);
	}
	...
	public List<Asset> getAll() {
		return createQueryWithCache("From Asset").getResultList();
	}
	...
	public List<Asset> getByPageAndSize(Integer pageIndex, Integer pageSize) {
		return createQueryWithCache("From Asset").setMaxResults(pageSize).setFirstResult(pageSize * (pageIndex - 1)).getResultList();
	}
	...
	public List<Asset> getFromAnalysisByPageAndSize(Integer analysisId, Integer pageIndex, Integer pageSize) {
		...
		return createQueryWithCache(query).setParameter("analysisId", analysisId).setMaxResults(pageSize).setFirstResult(pageSize * (pageIndex - 1)).getResultList();
	}
	...
	public List<Asset> getAllFromAnalysis(Integer analysisId) {
		...
		return createQueryWithCache(query).setParameter("analysisId", analysisId).getResultList();
	}
	...
	public Integer getAnalysisIdFromAsset(Integer assetId) {
		return (Integer) createQueryWithCache("SELECT analysis.id From Analysis analysis join analysis.assets asset where asset.id = :assetID").setParameter("assetID", assetId)
				.getSingleResult();
	}
	...
	public Asset getByNameAndAnlysisId(String name, int idAnalysis) {
		return (Asset) createQueryWithCache("Select asset From Analysis as analysis inner join analysis.assets as asset where analysis.id = :idAnalysis and asset.name = :name")
				.setParameter("idAnalysis", idAnalysis).setParameter("name", name).getSingleResult();
	}
