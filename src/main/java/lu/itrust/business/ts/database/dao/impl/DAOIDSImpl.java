 ...
 */
@Repository
public class DAOIDSImpl extends DAOHibernate implements DAOIDS {
	 ...
	 * @param session
	 */
	public DAOIDSImpl(Session session) {
		super(session);
	}
	...
	public IDS get(String prefix) {
		return (IDS) createQueryWithCache("From IDS where prefix = :prefix").setParameter("prefix", prefix).getSingleResult();
	}
	...
	public List<IDS> getByAnalysisId(int idAnalysis) {
		return createQueryWithCache("Select ids From IDS ids inner join ids.subscribers as subscriber  where ids.enable = true and subscriber.id = :idAnalysis order by ids.prefix")
	...
	public List<IDS> getAllByState(boolean enabled) {
		return createQueryWithCache("From IDS ids where ids.enable = :enabled order by ids.prefix").setParameter("enabled", enabled).getResultList();
	}

	@SuppressWarnings("unchecked")
	@Override
	public List<IDS> getAllNoSubscribers() {
		return createQueryWithCache("From IDS ids where ids.subscribers IS EMPTY order by ids.prefix").getResultList();
	}

	@SuppressWarnings("unchecked")
	@Override
	public List<IDS> getAll() {
		return createQueryWithCache("From IDS order by prefix").getResultList();
	}

	@SuppressWarnings("unchecked")
	@Override
	public List<String> getPrefixesByAnalysisId(int idAnalysis) {
		return createQueryWithCache("Select ids.prefix From IDS ids inner join ids.subscribers as subscriber  where ids.enable = true and subscriber.id = :idAnalysis order by ids.prefix")
				.setParameter("idAnalysis", idAnalysis).getResultList();
	}
	...
	public boolean existByPrefix(String prefix) {
		return (boolean) createQueryWithCache("Select count(*)> 0 From IDS where prefix = :prefix").setParameter("prefix", prefix).getSingleResult();
	}

	@Override
	public boolean exists(String token) {
		return (boolean) createQueryWithCache("Select count(*)> 0 From IDS where token = :token").setParameter("token", token).getSingleResult();
	}

	@Override
	public void delete(Integer id) {
		createQueryWithCache("Delete IDS where id = :id").setParameter("id", id).executeUpdate();
	}
	...
	public IDS getByToken(String token) {
		return (IDS) createQueryWithCache("From IDS ids where ids.token = :token ").setParameter("token", token).uniqueResultOptional().orElse(null);
	}

	@Override
	public boolean exists(boolean state) {
		return createQueryWithCache("Select count(*)> 0 From IDS ids where ids.enable = :enabled", Boolean.class).setParameter("enabled", state).uniqueResultOptional()
				.orElse(false);
	}

	@Override
	public Stream<Integer> findSubscriberIdByUsername(String username) {
		return createQueryWithCache("Select distinct (subscriber.id) From IDS ids inner join ids.subscribers as subscriber where ids.prefix = :prefix", Integer.class)
