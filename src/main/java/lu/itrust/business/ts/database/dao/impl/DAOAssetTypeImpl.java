 ...
 */
@Repository
public class DAOAssetTypeImpl extends DAOHibernate implements DAOAssetType {

	/**
	 * Constructor: <br>
	 */
	public DAOAssetTypeImpl() {
	}
	 ...
	 * @param session
	 */
	public DAOAssetTypeImpl(Session session) {
		super(session);
	}
	...
	public AssetType getByName(String name)  {
		return (AssetType) createQueryWithCache("From AssetType where name= :name").setParameter("name", name).uniqueResultOptional().orElse(null);
	}
	...
	public List<AssetType> getAll()  {
		return createQueryWithCache("From AssetType order by name",AssetType.class).getResultList();
	}
