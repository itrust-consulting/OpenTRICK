 ...
 */
@Repository
public class DAORiskAcceptanceParameterHBM extends DAOHibernate implements DAORiskAcceptanceParameter {
	...
	public boolean exists(Integer id) {
		return getSession().createQuery("Select count(parameter)>0 From RiskAcceptanceParameter as parameter where parameter.id = :id", Boolean.class).setParameter("id", id)
	...
	public List<RiskAcceptanceParameter> findAll() {
		return getSession().createQuery("From RiskAcceptanceParameter", RiskAcceptanceParameter.class).getResultList();
	}
	...
	public List<RiskAcceptanceParameter> findAll(List<Integer> ids) {
		...
		return getSession().createQuery("From RiskAcceptanceParameter where id in (ids) order by value,color, label, description", RiskAcceptanceParameter.class)
	...
	@Override
	public long count() {
		return getSession().createQuery("Select count(*) From RiskAcceptanceParameter", Long.class).uniqueResultOptional().orElse(0L);
	}
	...
	public void delete(Integer id) {
		getSession().createQuery("Delete From RiskAcceptanceParameter where id = :id").setParameter("id", id).executeUpdate();
